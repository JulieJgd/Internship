...
for i in list_col:
    if os.path.exists("results_" + i + ".csv"):
        os.remove("results_" + i + ".csv")

for i in list_col:  # os.listdir(path)
    # path_1 = os.path.join(path, i)
    data_bt = pd.read_csv(i + "/benign_traffic.csv")
    data_a = pd.read_csv(i + "/attacks.csv")
    columns = data_bt.columns
    data = data_a.append(data_bt).sample(frac=1)
    y = data[["Malware"]]
    ...
    classes = data.Malware.value_counts().to_dict()
    ...
    key = [k for k, v in classes.items() if v == minor_class_count][0]
    # print(key)
    ...
    y_GAN_pred = clf_GAN.predict(X_GAN_test)
    ...
    X_SMOTE_train, X_SMOTE_test, y_SMOTE_train, y_SMOTE_test = train_test_split(X_SMOTE, y_SMOTE, random_state=0,
                                                                                test_size=0.3)
    clf_SMOTE = LinearSVC().fit(X_SMOTE_train, y_SMOTE_train)
    y_SMOTE_pred = clf_SMOTE.predict(X_SMOTE_test)
    ...
    #
    # print("GAN", i)
    # print("Accuracy of new data model:", accuracy_score(y_GAN_test, y_GAN_pred))
    # print("Precision:", precision_score(y_GAN_test, y_GAN_pred))
    # print("Classification report of new data model: \n", classification_report(y_GAN_test, y_GAN_pred))
    #
    # print("SMOTE", i)
    # print("Accuracy of real data model:", accuracy_score(y_SMOTE_test, y_SMOTE_pred))
    ...
    df_report_GAN = pandas_classification_report(y_GAN_test, y_GAN_pred, test="GAN")
    df_report_SMOTE = pandas_classification_report(y_SMOTE_test, y_SMOTE_pred, test="SMOTE")
    df_report_full = df_report.append(df_report_GAN)
    df_report_full = df_report_full.append(df_report_SMOTE)
    # print(df_report)

    df_report_full.to_csv('results_' + i + '.csv', sep=';')
